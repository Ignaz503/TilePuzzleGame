...
public class TestLevel : Level
{
    public TestLevel(int width, int height) : base(
        ...
        {
            new MoveableTileSpawnInfo(){ GridPosition = Vector2Int.zero, Value = 2, SerializedMergeEffect = new SpawnBlockerMergeEffect().Serialize() },
            new MoveableTileSpawnInfo(){GridPosition = Vector2Int.up,Value =2, SerializedMergeEffect = new SpawnBlockerMergeEffect().Serialize()},
            new MoveableTileSpawnInfo(){GridPosition = Vector2Int.right,Value =1},
            new MoveableTileSpawnInfo(){GridPosition = new Vector2Int(3,2),Value =2, SerializedMergeEffect = new SpawnBlockerMergeEffect().Serialize()},
            new MoveableTileSpawnInfo(){GridPosition = new Vector2Int(4,4),Value =3, SerializedMergeEffect = new SpawnBlockerMergeEffect().Serialize()},
            new MoveableTileSpawnInfo(){GridPosition = new Vector2Int(2,4),Value =0, SerializedMergeEffect = new SpawnBlockerMergeEffect().Serialize()}
        }),
